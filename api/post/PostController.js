...
async function deletePost(req, res) {
    const id = req.params.id
    const post = req.body
    if (post.owner._id !== req.session.user._id) return Promise.reject('You are not the owner of the post')
    try {
...
async function updatePost(req, res) {
    const post = req.body;
    if (post.owner._id !== req.session.user._id) return res.status(500).send('Could Not Edit')
    try {
